...
void findAndPrintLongestPath(char **Map, int rows, int cols, int x, int y, int endX, int endY, int length, int (*visited)[MAX_COLS], Path *currentPath, Path *longestPath) {
    if (x == endX && y == endY) {
        if (length > longestPath->length) {
            longestPath->length = length;
            memcpy(longestPath->path, currentPath->path, length * sizeof(currentPath->path[0]));
        }
        return;
    }
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        if (newX >= 0 && newX < rows && newY >= 0 && newY < cols && !visited[newX][newY] && (Map[newX][newY] == '.' || Map[newX][newY] == 'E')) {
            visited[newX][newY] = 1;
            currentPath->path[length][0] = newX;
            currentPath->path[length][1] = newY;
            findAndPrintLongestPath(Map, rows, cols, newX, newY, endX, endY, length + 1, visited, currentPath, longestPath);
            visited[newX][newY] = 0; // Backtrack
        }
    }
}
...
int main() {
    ...
    if (dp[endX][endY].length == INF) {
        ...
        currentPath.path[0][0] = startX;
        currentPath.path[0][1] = startY;
        findAndPrintLongestPath(Map, rows, cols, startX, startY, endX, endY, 1, visited, &currentPath, &longestPath);
